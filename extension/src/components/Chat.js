const Chat = ({response, text, isLoading}) => {
    return (
        <div className={`flex ${response ? '' : 'flex-row-reverse'} items-end gap-2`}>
            ...
            <div className={`message shadow-2xl rounded-3xl ${response ? 'bg-white text-black' : 'bg-purple-400 text-white'} text-sm sm:text-base p-4 px-5 max-w-[300px] sm:max-w-[300px]`}>
                { isLoading ? (
